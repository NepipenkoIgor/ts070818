...
function summator(...params: paramsType[]): number {
    ...
    for (let i: number = 0; i < params.length; i++ ) {
        const param: paramsType = params[i];

        if (typeof param === `number`) {
            paramsSum += param;
        } else {
            const changeToNumber: number = +param;
...
console.log(`Проверка, сумма = 10`, summator(`5`, 5, `wow`));
...
function getUnique<T> (...params: T[]): T[] {
    ...
    function onlyUnique(value: T, index: number, array: T[]): any {
        return array.indexOf(value) === index;
    }

    const filterArrElements: T[] = params.filter(onlyUnique);

    return filterArrElements;
}
